...
function replayLinks() {
  return [
    {
      label: 'Agent Flow · 4장 실행',
      url: 'https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch04',
      lens: '원인, tool use/result, interrupt, unmapped claim',
    },
    {
      label: 'Runtime City · 4장 실행',
      url: 'https://nfbs2000.github.io/speaky-PGSimCity/education/claude-sdk-runtime-city/?run=book-sdk-live-20260803',
      lens: 'OTel 시간축, 실행 zone, Observed/Missing',
    },
    {
      label: 'Pixel Agents · 4장 증거',
      url: 'https://nfbs2000.github.io/speaky-pixel-agents/book-sdk-evidence/',
      lens: 'actor와 tool lifecycle 상태',
    },
    {
      label: 'Hypnagonia · Observation Ordeal',
      url: 'https://nfbs2000.github.io/speaky-hypnagonia/book-sdk-ordeal/',
...
async function verify(markdownPath, jsonPath) {
  ...
  if (value.claims.filter((claim) => claim.status === 'observed').length !== 4) throw new Error('observed_claim_count_invalid')
  if (value.claims.filter((claim) => claim.status === 'additional_observation_required').length !== 2) {
