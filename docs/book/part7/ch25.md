...
Actor, Projector, Judge를 분리하고 파생 그래프를 모델의 숨은 사고로 부르지 않는 원칙은 이 책 전체의 핵심입니다. 직접 실행한 Actor 코드가 사실을 만들고 OTel이 사건 관계를 보존하며 Speaky가 이를 투영하고, Judge는 고정된 범위만 읽을 때 책이 진실에 가까워집니다. 관찰팩은 이 결합을 위한 것이지 hash로 진실을 선언하기 위한 것이 아닙니다. 예제는 하나의 actor output을 고정한 뒤 deterministic projector와 독립 rubric judge를 순서대로 실행하고 각 입력 범위를 출력하도록 개선해야 합니다. [평가자-최적화자 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/patterns/agents/evaluator_optimizer_kr.html)이 생성자와 평가자 분리의 익숙한 출발점이며, 이 장의 단방향 경계는 [Speaky Agent Flow 25장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch25)에서 확인할 수 있습니다.
