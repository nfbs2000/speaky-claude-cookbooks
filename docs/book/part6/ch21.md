...
effort, Fast Mode, thinking을 같은 개념으로 묶지 말아야 한다는 방향은 옳습니다. 공개 SDK와 관찰팩이 확인할 수 있는 것은 설정, 응답 모델, usage, 지연, 도구 행동, 종료 상태이며 내부 사고의 품질 자체는 아닙니다. 현재 실행은 effort 조건 일부만 다루고 Fast Mode는 꺼져 있었으므로 Speaky도 속도 향상을 그려서는 안 됩니다. 예제는 thinking을 켠 Messages API 도구 호출, effort A/B, Fast Mode를 각각 독립 실행하고 공개 content block과 usage만 비교하도록 개선해야 합니다. [도구 사용과 확장 사고 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/extended_thinking/extended_thinking_with_tool_use_kr.html)이 content block 보존의 실행 기준이며, 현재 관찰 범위는 [Speaky Agent Flow 21장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch21)에서 확인할 수 있습니다.
