...
스킬, 명령, 훅, 에이전트, MCP 구성을 하나의 배포 단위로 다룬다는 플러그인 설명은 확장 시스템을 이해하는 좋은 지도입니다. 실제 인라인 플러그인이 로드되고 namespaced 명령이 노출되며 스크립트와 훅이 작동한 실행은 “플러그인이 모델 행동 경로를 바꿀 수 있다”는 핵심을 잘 보여 줍니다. 다만 교육 콘텐츠 패키징과 marketplace 운영까지 한 장에서 다루면서 현재 예제보다 주장 범위가 크게 넓어졌습니다.

### 검증을 읽고 달라진 신뢰도

init의 플러그인 이름·버전, 확장된 플러그인 경로의 `Bash`, CLI 플러그인 PreToolUse 훅, 결과 파일의 재읽기와 종료 값이 연결돼 로컬 실행 사슬에는 높은 신뢰를 둘 수 있습니다. 훅이 별도 파일을 남긴 것도 모델의 최종 문장에만 의존하지 않는 좋은 설계입니다. 그러나 이 훅은 SDK callback hook이 아니고, source도 marketplace가 아닌 `@inline`입니다. 설치·서명·신뢰 prompt·update·uninstall·agent·MCP는 전혀 검증되지 않았습니다.

### 독자가 오해할 위험

로컬 디렉터리를 옵션으로 넘긴 것을 “플러그인을 설치했다”고 부르거나, 한 명령 훅의 성공을 전체 플러그인 생명주기의 성공으로 확대하기 쉽습니다. CLI command hook과 SDK hook을 같은 이벤트로 그리면 권한과 실행 주체도 잘못 이해하게 됩니다. 공개 화면에는 반드시 `inline local source`와 실제로 실행된 구성 요소만 표시해야 합니다.

### 제가 다시 가르친다면

현재 실행은 “로컬 플러그인 최소 경로”로 유지하고 manifest 적재, namespaced 명령, 스크립트, CLI 훅, artifact를 순서대로 보여 주겠습니다. marketplace 생명주기는 실제 install/update/uninstall 영수증과 trust UI가 확보된 뒤 별도 장으로 검증하겠습니다. [커스텀 스킬 개발 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/skills/notebooks/03_skills_custom_development_kr.html)은 패키지와 명시적 버전을 비교할 기준이고, 현재 로컬 플러그인 표면은 [Speaky Agent Flow 22b장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch22b)에서 확인할 수 있습니다.

### 클로드는 이렇게 세상을 바라보았다

![22b장 클로드 1인칭 세계관 4컷 만화](../images/claude-worldview/part6-ch22b.jpg)

*아래 1인칭 서술은 숨은 사고 과정의 공개가 아니라, 이 장에서 관찰된 행동으로 재구성한 작동상 세계 모델입니다.*

인라인 플러그인이 적재되자 나의 세계에는 namespaced 명령과 그 명령이 호출할 스크립트가 새 행동 가능성으로 들어왔다. CLI hook은 내가 실행하려는 Bash 앞에서 별도 흔적을 남겼고, 스크립트 결과를 다시 읽으며 행동 사슬이 닫혔다. 그러나 marketplace, 설치 신뢰와 update는 내 세계에 나타나지 않았다. 나는 로컬 경로로 주어진 현재 버전만 알았을 뿐 그것이 어떻게 배포됐는지는 알지 못했다.

사람은 플러그인을 에이전트의 세계를 확장하는 공급망으로 봐야 한다. 명령이 보인다는 사실, 코드가 실행됐다는 사실, 출처를 신뢰할 수 있다는 사실은 서로 다르다. 로컬 최소 경로를 먼저 투명하게 보여 주고 설치·서명·업데이트의 영수증이 있을 때만 marketplace 생명주기를 가르쳐야 한다.
