...
Skill을 재사용 가능한 절차·코드·리소스 패키지로 보는 설명은 공식 기법과 일치합니다. 하지만 시작 목록 노출, 파일 읽기, `Skill` 도구 실행은 서로 다른 사건이고, 로컬 Agent SDK skill과 Messages API skill/files beta도 분리해야 합니다. 관찰팩은 skill 전용 표식과 실제 산출물이 생긴 경우에만 사용을 표시해야 합니다. 예제는 최소 `SKILL.md` 하나를 만들고 적재 목록, skill 호출, 생성 artifact를 차례로 출력한 뒤 두 API 계열을 별도 파일로 제공하도록 개선하는 것이 좋습니다. [커스텀 스킬 개발 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/skills/notebooks/03_skills_custom_development_kr.html)이 패키지·업로드·실행·버전의 기준이며, 이 장의 로컬 실행은 [Speaky Agent Flow 22장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch22)에서 확인할 수 있습니다.
