...
이 장의 3계층 구분은 Agent SDK를 이해하는 출발점으로 타당합니다. 다만 원문만으로는 왜 별도의 관찰팩이 필요한지 드러나지 않습니다. `book_sdk_proof`의 최소 `query()` 실행이 세션 시작, `Read` 요청·결과, 종료를 실제 코드로 만들고, 같은 실행의 OTel을 Speaky가 시간선으로 재생할 때 비로소 “코드에서 발생한 사실”과 “제품 아키텍처 해석”을 구분할 수 있습니다. 예제는 현재의 저장·manifest 코드보다 먼저 20줄 안팎의 `query()`와 메시지 출력부터 보여 주는 편이 좋습니다. 익숙한 출발점은 [한 줄 리서치 에이전트 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/claude_agent_sdk/00_The_one_liner_research_agent_kr.html), 이 장의 코드+OTel 결합은 [Speaky Agent Flow 1장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch01)에서 확인할 수 있습니다.
