...
읽기 전용 작업의 병렬화, 변경 작업의 직렬화, 인터럽트 전파라는 설계 원칙은 유효하지만, 현재 직접 실행이 확인한 범위는 annotation을 준 통제 MCP 도구입니다. 관찰팩은 각 호출의 시작·종료·권한·취소를 실제 코드와 시각 시간축으로 연결하려고 존재하며, 이것을 built-in 도구 전체의 내부 스케줄러 증명으로 확대하면 안 됩니다. 예제는 두 개의 지연 도구를 순차·병렬 조건으로 각각 실행하고 wall-clock과 tool ID만 비교한 뒤, 인터럽트를 별도 사례로 분리하는 편이 좋습니다. [병렬 도구 호출 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/tool_use/parallel_tools_kr.html)이 왕복과 batch 도구 차이를 보여 주고, 이 장의 코드+OTel 시간선은 [Speaky Agent Flow 4장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch04)에서 확인할 수 있습니다.
