...
## 실제 Python SDK 관찰이 추가됐다

이 장은 이제 cookbook 링크만으로 설명하지 않는다. `claude-agent-sdk 0.2.128`, Claude Code
`2.1.220`, `claude-opus-5`로 세 case를 실제 순차 실행하고 raw SDK message와 host process
timing을 저장했다. 원천 95개 사건에서 공개에 필요한 32개만 투영했으며 source event ID와
SHA-256을 유지했다.

- read-only custom MCP 두 handler: **858.949ms overlap 관찰**
- mutating custom MCP 두 handler: **overlap 0ms, 첫 종료 뒤 두 번째 시작 관찰**
- 완성된 ToolUseBlock의 handler: **assistant `message_stop` 전에 시작 관찰**
- `ClaudeSDKClient.interrupt()`: **handler cancelled, `aborted_streaming` 관찰**
- built-in·annotation 없는 도구와 progress/file persistence: **추가 관측 필요**

[4장 실제 Python SDK 관찰 원장](../evidence/ch04-live.md)에는 attempt ID, model, raw SDK/process
hash, invalid attempt와 네 개 공개 replay 링크가 들어 있다. 관찰되지 않은 사건은 책이
틀렸다는 뜻으로 바꾸지 않는다.
