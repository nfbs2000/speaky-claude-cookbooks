...
압축 뒤 파일·skill·계획을 다시 확인해야 한다는 운영 원칙은 옳습니다. 실제 코드에서는 파일이 바뀌었는데 새 `Read`가 없었고 모델은 다시 읽었다고 잘못 보고했으므로, “요약에 남음”, “사용 가능함”, “현재 디스크 재조회”를 분리해야 합니다. 관찰팩은 이 차이를 Speaky 시간선에서 파일 변경과 `Read` 부재로 보여 주기 위해 존재합니다. 예제는 압축 직후 호스트가 fixture를 V1에서 V2로 바꾸고, 모델 응답과 실제 `Read` 결과를 나란히 출력하도록 개선하는 것이 좋습니다. [컨텍스트 엔지니어링 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/tool_use/context_engineering/context_engineering_tools_kr.html)이 컴팩션·정리·메모리를 구분하는 기준이고, stale 동작은 [Speaky Agent Flow 10장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch10)에서 확인할 수 있습니다.
