...
도구 설명을 작은 하니스로 보는 관점은 공식 tool-use 예제와 잘 맞습니다. 관찰팩은 모델이 어떤 도구와 인자를 실제로 선택했는지 보여 주지만, 좋은 결과 한 건만으로 설명 문구의 효과를 증명하지는 못합니다. 예제는 같은 schema와 task를 유지한 채 description만 좋은 버전·빈 버전으로 바꾸고 선택률, 인자 오류, 재시도 횟수를 비교하도록 개선해야 합니다. [도구 평가 노트북](https://nfbs2000.github.io/speaky-claude-cookbooks/notebooks/tool_evaluation/tool_evaluation_kr.html)은 빈 설명이 불필요한 호출과 오류를 늘리는 과정을 실제 코드로 보여 주며, 이 장의 실행 사건은 [Speaky Agent Flow 8장 재생](https://nfbs2000.github.io/speaky-agent-flow/education/?collection=book-sdk-ko&run=ch08)에서 확인할 수 있습니다.
