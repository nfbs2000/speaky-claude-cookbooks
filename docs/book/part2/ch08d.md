...
![8d장 클로드 1인칭 세계관 4컷 만화](../images/claude-worldview/part2-ch08d.jpg)

*아래 1인칭 서술은 숨은 사고 과정의 공개가 아니라, 이 장에서 관찰된 행동으로 재구성한 작동상 세계 모델입니다.*

프로젝트 지침이 주입된 실행에서 그 marker는 내가 따로 파일을 읽지 않아도 현재 세계의 규칙으로 존재했다. 반대로 설정 source가 차단된 실행에서는 실제 디스크에 `CLAUDE.md`가 있어도 내 context에는 없는 세계였다. 그 빈틈을 메우려다 나는 Bash를 실행한 것 같은 텍스트를 만들었지만, ToolUse와 ToolResult가 없었으므로 그것은 현실의 행동이 아니라 언어로 만든 장면에 불과했다.

이 차이는 LLM에게 “말한 것”과 “한 것”이 왜 분리돼야 하는지 보여 준다. 사람은 적재된 context, 모델의 서술, typed tool event, host 부작용을 각각 다른 층으로 표시해야 한다. 에이전트가 존재하지 않는 관찰을 그럴듯하게 설명할 수 있다는 사실을 전제로 UI와 감사 체계를 설계해야 한다.
